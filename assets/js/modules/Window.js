export default class Window {
	...
	constructor(options={}) {
		...
		$$.UI.Desktop.elements.el.append($$.Tools.autoAppend(win.elements));
	...
	applySize(){
		...
		if(win.options.maximized || $$.System.isMobile()){
			win.elements.el.css({width: $$.UI.Desktop.width()+'px',height: $$.UI.Desktop.height()+'px'});
		}
		else {
	...
	resize(event,direction){
		...
		$(document).on('mousemove.window',function(event){
			...
			if(direction[0] === 'e' || direction[1] === 'e'){
				...
				//prevent resize out of desktop
				if(win.options.left.current+win.options.width.current >= $$.UI.Desktop.width()){win.options.width.current = $$.UI.Desktop.width()-win.options.left.current;}
				//check min width
			...
			if(direction[0] === 's' || direction[1] === 's'){
				win.options.height.current = win.options.height.start+offsetY;
				//prevent resize out of desktop
				if(win.options.top.current+win.options.height.current >= $$.UI.Desktop.height()){win.options.height.current = $$.UI.Desktop.height()-win.options.top.current;}
				//check min height
	...
	drag(event){
		...
		$(document).on('mousemove.window',function(event){
			...
			else if(win.options.top.current+win.options.height.current >= $$.UI.Desktop.height()) {win.options.top.current = $$.UI.Desktop.height() - win.options.height.current;}
			if(win.options.left.current < 0){win.options.left.current = 0;}
			else if(win.options.left.current+win.options.width.current >= $$.UI.Desktop.width()){win.options.left.current = $$.UI.Desktop.width()-win.options.width.current}
			win.applyPosition();
		})
		$(document).one('mouseup',function(){
			$(document).off('mousemove.window');
	...
	bringToFront(){
		let win = this;
		$$.UI.Taskbar.removeActive();
		win.taskbarIcon.addClass('active');
		if(win.elements.el.css('z-index') != $$.UI.zindex){
			win.elements.el.css({zIndex:++$$.UI.zindex});
		}
	}
	close(){
		let win = this;
	...
	toggle(){
		let win = this;
		if(win.elements.el.css('display') == 'none' || win.elements.el.css('z-index') != $$.UI.zindex){
			win.restore();
		}
		else {
			win.minimize();
	...
	register(){
		let win = this;
		win.taskbarIcon = $$.UI.Taskbar.add(win.options.icon,win.options.title);
